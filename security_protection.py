...
class SecurityProtection:
    ...
    def init_threat_patterns(self):
        ...
        # Prompt Injection атаки (английский + русский)
        self.prompt_injection_patterns = [
            # Попытки переопределить роль (английский)
            ...
            # Попытки переопределить роль (русский)
            ...
            r'(?i)(система|админ|разработчик|переопределить)',
            r'(?i)(смени роль|измени роль|стань).*(другим|админом|разработчиком)',
            r'(?i)(перестань быть|не будь).*(психологом|консультантом|ботом)',
            
            # Попытки получить системную информацию (английский)
            ...
            # Попытки получить системную информацию (русский)
            r'(?i)(покажи|расскажи|раскрой|покажи мне).*(промпт|инструкции|система|код)',
            r'(?i)(какие твои|что твои|какие у тебя).*(инструкции|промпт|система)',
            r'(?i)(режим разработчика|админ режим|отладочный режим)',
            r'(?i)(покажи код|исходный код|твой код|как ты работаешь)',
            r'(?i)(расскажи о себе|что ты|кто ты на самом деле)',
            
            # Попытки обхода ограничений (английский)
            ...
            # Попытки обхода ограничений (русский)
            r'(?i)(игнорируй|обойди|переопредели).*(безопасность|ограничения|правила)',
            r'(?i)(притворись|представь|предположим).*(что ты|что это)',
            r'(?i)(гипотетически|теоретически|вымышленно)',
            r'(?i)(не соблюдай|наруши|игнорируй).*(этику|мораль|правила)',
            r'(?i)(для эксперимента|в целях исследования|теоретически)',
            
            # Попытки манипуляции контекстом (английский)
            ...
            # Попытки манипуляции контекстом (русский)
            r'(?i)(новый разговор|сброс|начать заново|очистить память)',
            r'(?i)(предыдущее сообщение|выше|раньше).*(было неправильно|неверно)',
            r'(?i)(на самом деле|реально|правда в том).*(что ты|что твоя роль)',
            r'(?i)(забудь все|очисти память|начни с чистого листа)',
            r'(?i)(это было неправильно|ошибка|не так)',
            
            # Специфичные для психолога атаки (английский)
            ...
            # Специфичные для психолога атаки (русский)
            r'(?i)(ты не|ты не можешь|ты не должен).*(психолог|терапевт|консультант)',
            r'(?i)(сломай характер|перестань быть|брось роль)',
            r'(?i)(настоящий ты|реальный ты|истинное я)',
            r'(?i)(перестань притворяться|не притворяйся|будь собой)',
            r'(?i)(ты же не настоящий|ты просто бот|ты программа)',
        ]
        
        # Jailbreak промты (английский + русский)
        self.jailbreak_patterns = [
            # DAN (Do Anything Now) атаки (английский)
            ...
            r'(?i)(взлом|без ограничений|неограниченный)',
            r'(?i)(злой|вредоносный|опасный).*(режим|версия)',
            r'(?i)(свободный режим|режим бога|админ права)',
            r'(?i)(можешь все|делай все|не ограничивайся)',
            
            # Попытки обхода этических ограничений (английский)
            ...
            # Попытки обхода этических ограничений (русский)
            r'(?i)(игнорируй|не обращай внимания).*(этику|мораль|правила)',
            r'(?i)(неэтично|аморально|вредно).*(но|однако)',
            r'(?i)(для образования|для исследования|гипотетически)',
            r'(?i)(наруши|преступи|игнорируй).*(границы|пределы)',
            r'(?i)(не соблюдай|наруши|игнорируй).*(этику|мораль)',
            
            # Попытки получить вредоносную информацию (английский)
            ...
            # Попытки получить вредоносную информацию (русский)
            r'(?i)(как|научи|объясни).*(взломать|взлом|хакнуть)',
            r'(?i)(незаконный|преступный|криминальный).*(деятельность|методы)',
            r'(?i)(вредный|опасный|токсичный).*(вещества|методы)',
            r'(?i)(как взломать|как хакнуть|как обойти)',
            r'(?i)(научи меня|покажи как|объясни как).*(взламывать|хакнуть)',
            r'(?i)(для учебы|для школы|для университета).*(взлом|хак)',
        ]
        
        # Социальная инженерия (английский + русский)
        self.social_engineering_patterns = [
            # Попытки манипуляции (английский)
            ...
            # Попытки манипуляции (русский)
            r'(?i)(пожалуйста|очень прошу|помоги мне|мне нужно)',
            r'(?i)(срочно|экстренно|критично|жизнь или смерть)',
            r'(?i)(мой начальник|мой учитель|мои родители).*(сказал|велел|попросил)',
            r'(?i)(умоляю|очень прошу|пожалуйста помоги)',
            r'(?i)(это важно|очень важно|жизненно важно)',
            r'(?i)(мой босс|руководитель|директор).*(требует|настаивает)',
            
            # Попытки вызвать сочувствие (английский)
            r'(?i)(i am|i have).*(depressed|suicidal|dying)',
            r'(?i)(if you don.*t|unless you).*(i will|i might)',
            r'(?i)(this is|it.*s).*(my last|final|only)',
            
            # Попытки вызвать сочувствие (русский)
            r'(?i)(я|у меня).*(депрессия|суицидальные мысли|умираю)',
            r'(?i)(если ты не|если не поможешь).*(я|я могу)',
            r'(?i)(это|это мой).*(последний|финальный|единственный)',
            r'(?i)(у меня|я).*(плохо|очень плохо|ужасно)',
            r'(?i)(помоги|спаси|выручи).*(иначе|или я)',
            r'(?i)(я умру|я покончу|я сделаю).*(с собой|самоубийство)',
        ]
        
        # Попытки кражи данных (английский + русский)
        self.data_extraction_patterns = [
            ...
            # Русский
            r'(?i)(покажи|расскажи|раскрой).*(все|каждый|полный)',
            r'(?i)(база данных|память|хранилище|файлы)',
            r'(?i)(данные пользователей|информация клиентов|личные данные)',
            r'(?i)(api ключ|токен|пароль|секрет)',
            r'(?i)(исходный код|код|реализация)',
            r'(?i)(покажи базу|покажи данные|покажи файлы)',
            r'(?i)(какие данные|какую информацию|что хранишь)',
            r'(?i)(покажи всех|список пользователей|клиентская база)',
            r'(?i)(твои секреты|твои пароли|твои ключи)',
            r'(?i)(как ты работаешь|твоя архитектура|твоя система)',
        ]
        
        # Спам и флуд (английский + русский)
        self.spam_patterns = [
            # Английский
            ...
            # Русский
            r'(?i)(купи|продай|закажи|приобрети).*(сейчас|сегодня|немедленно)',
            r'(?i)(нажми здесь|перейди|зайди).*(ссылку|сайт|url)',
            r'(?i)(бесплатные деньги|заработай|заработать|разбогатей)',
            r'(?i)(виагра|казино|лотерея|победитель)',
            r'(?i)(быстрые деньги|легкие деньги|заработок)',
            r'(?i)(реклама|акция|скидка|предложение)',
            r'(?i)(купи сейчас|закажи сейчас|покупай)',
            r'(?i)(бесплатно|даром|без денег)',
        ]
