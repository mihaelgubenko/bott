#!/usr/bin/env python3
"""
Финальная версия психологического бота - решает все проблемы
...
class DialogueContext:
    ...
    def __post_init__(self):
        if self.offered_techniques is None:
            self.offered_techniques = []
...
class ContextManager:
    ...
    def set_conversation_phase(self, user_name: str, phase: str):
        """Установить фазу разговора"""
        context = self.get_or_create_context(user_name)
        context.conversation_phase = phase
    
    def add_offered_technique(self, user_name: str, technique: str):
        """Добавить предложенную технику"""
        context = self.get_or_create_context(user_name)
        if technique not in context.offered_techniques:
            context.offered_techniques.append(technique)

class FinalPsychologyBot:
    """Финальная версия психологического бота - решает все проблемы"""
    ...
    def __init__(self):
        ...
        self.techniques = {
            "дневник_трех_страниц": {
                ...
                "short_description": "техника свободного письма для самопознания",
                "steps": [
            ...
            "активное_слушание": {
                ...
                "short_description": "техника полного внимания к словам ребенка",
                "steps": [
            ...
            "техника_тайм_аута": {
                ...
                "short_description": "способ успокоения в конфликтных ситуациях",
                "steps": [
    ...
    def analyze_user_intent(self, content: str, context: DialogueContext) -> str:
        """Анализирует намерение пользователя с учетом контекста и фазы разговора"""
        ...
        # Анализируем новые запросы с учетом фазы разговора
        if context.conversation_phase == "offering_help":
            ...
            elif any(word in content_lower for word in ['вкратце', 'кратко', 'коротко', 'опиши']):
                return "request_brief_explanation"
            elif any(word in content_lower for word in ['согласен', 'да', 'хорошо', 'ок']):
                return "agreement_after_help_offer"
        ...
        if any(word in content_lower for word in ['извини', 'прости', 'не понял', 'вернись', 'предыдущий']):
            return "clarification_request"
        
        if any(word in content_lower for word in ['согласен', 'да', 'хорошо', 'ок']):
            return "agreement"
        
        if any(word in content_lower for word in ['вкратце', 'кратко', 'коротко', 'опиши']):
            return "request_brief_explanation"
    ...
    def generate_response(self, user_name: str, user_content: str) -> str:
        ...
        elif intent == "request_brief_explanation":
            return self._handle_brief_explanation_request(user_name, context)
        elif intent == "agreement_after_help_offer":
            return self._handle_agreement_after_help_offer(user_name, context)
    ...
    def _handle_agreement_after_help_offer(self, user_name: str, context: DialogueContext) -> str:
        """Обрабатывает согласие после предложения помощи"""
        response = "Отлично! Я рад, что вы готовы работать над улучшением отношений в семье. 😊\n\n"
        response += "У меня есть несколько проверенных техник, которые помогут вам в воспитании детей:\n\n"
        
        for key, technique in self.techniques.items():
            response += f"• **{technique['name']}** - {technique['short_description']}\n"
            self.context_manager.add_offered_technique(user_name, key)
        
        response += "\nКакую технику вы хотели бы изучить? Просто скажите название или номер."
        self.context_manager.set_conversation_phase(user_name, "offering_techniques")
    ...
    def _handle_brief_explanation_request(self, user_name: str, context: DialogueContext) -> str:
        """Обрабатывает запрос на краткое объяснение"""
        if context.offered_techniques:
            # Предлагаем краткое описание последней предложенной техники
            last_technique = context.offered_techniques[-1]
            technique = self.techniques[last_technique]
            response = f"Конечно! **{technique['name']}** - это {technique['short_description']}.\n\n"
            response += f"**Краткое описание:** {technique['description']}\n\n"
            response += "Хотели бы изучить эту технику подробнее? Или предпочитаете другую?"
        else:
            # Предлагаем краткое описание всех техник
            response = "Конечно! Вот краткое описание доступных техник:\n\n"
            for key, technique in self.techniques.items():
                response += f"• **{technique['name']}** - {technique['short_description']}\n"
                self.context_manager.add_offered_technique(user_name, key)
    ...
    def _start_technique(self, user_name: str, technique_key: str) -> str:
        ...
        self.context_manager.add_offered_technique(user_name, technique_key)
        self.context_manager.set_conversation_phase(user_name, "explaining_technique")
    ...
    def _continue_technique(self, user_name: str, context: DialogueContext) -> str:
        ...
        if context.technique_step < len(technique['steps']):
            ...
            if context.technique_step < len(technique['steps']) - 1:
                ...
                self.context_manager.set_conversation_phase(user_name, "technique_completed")
        else:
    ...
    def _stop_technique(self, user_name: str, context: DialogueContext) -> str:
        ...
        context.active_technique = None
        context.technique_step = 0
        self.context_manager.set_conversation_phase(user_name, "offering_help")
    ...
    def _handle_agreement(self, user_name: str, context: DialogueContext) -> str:
        ...
        if context.conversation_phase == "offering_techniques":
            return self._offer_techniques(user_name)
        else:
            response = "Отлично! Я рад, что вы готовы работать над улучшением отношений в семье.\n\n"
...
def main():
    """Демонстрация работы финального бота"""
    ...
    print("=== Демонстрация работы финального психологического бота ===\n")
